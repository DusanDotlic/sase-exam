...
<head>
  ...
  <title>Secure Notes – Log in</title>
  ...
  <style>
    ...
    .card { max-width: 460px; width: 92vw; background: #111827; border: 1px solid #374151; border-radius: 16px; padding: 28px; box-shadow: 0 10px 30px rgba(0,0,0,.35);}
...
<body>
  <main class="card">
    ...
    <p class="note">Don’t have an account? <a class="btn" href="/sase-exam/public/signup.php" style="margin-left:6px">Sign up</a></p>
  </main>
  ...
  <script>
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        const res = await fetch("/sase-exam/api/auth/login.php", {
          method: "POST",
        ...
        if (res.ok) {
          window.location.href = "/sase-exam/public/notes.php";
          return;
        }

        msg.textContent = (j && (j.error || j.message)) ? (j.error || j.message) : "Login failed.";
